...
const callGeminiWithFallback = async (contents) => {
  try {
    ...
    if (isRateLimitOrUnavailable) {
      console.warn('Gemini 2.5 Flash is unavailable/rate-limited. Falling back to Gemini 2.0 Flash...');
      try {
        return await ai.models.generateContent({
         ...
         if (error.status === 429 || fallbackError.status === 429 || (fallbackError.message && fallbackError.message.includes('quota'))) {
